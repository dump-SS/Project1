...
- 产出：一页 POC 结论——PaddleOCR 本地跑数学公式/中英文混排识别率数据 + 「进入下一迭代 / 延期到 **pilot 上线**后」结论；更新 `routes/ocr.py` docstring 与 backlog C4 状态。
...
### pilot 完成标准（原「上线门」，对齐 PRD 12.9；pilot 阶段不设门禁，未达标不阻塞推进）

T10/T11/T12 全绿 + D2/D4 **团队内部完成**（D2 监护人授权文案由团队自撰并在页面显著位置标注「学生团队开发，未经专业法律审核」；D4 出域边界 12.6 强化版由团队内部自查并留书面记录；正式法律意见与外部隐私评审签字均记入待办）+ 错题原文 100% 不出域断言绿（**CI 断言自动化记入待办**，不阻塞）。非代码项 D1-D4 沿用原细则第 6 节并行推进。
