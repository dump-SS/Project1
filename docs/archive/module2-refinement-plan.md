...
核心链路（知识库/错题本/mastery/复盘/出域合规）已跑通但**算法价值未真实生效**（embedding off、RAG 未接、调权未接），前端 5 个闭环缺口未补，验收类工作（压测/演示/降级矩阵）未执行——本迭代目标：**从"链路通"到"算法真生效 + 前端闭环 + 可 pilot 上线"**。
...
非代码项（D1-D4）随迭代并行；**〔pilot 口径〕原「法务两项卡 M3 上线门」取消门禁**——D2/D4 改为团队内部完成（D2 团队自撰 + 页面标注，D4 团队内部自查并留书面记录），未达标不阻塞 M3 推进。
...
### W3-2 AICallLog 持久化（B1，PRD 6.5 pilot 上线完成标准）
...
| W4-4 | 板块二隐私自查（原「隐私工程评审」） | EgressGuard 链路走查 + 出域断言复核 + 出域边界 12.6 强化版对照 | **〔pilot 口径〕团队内部自查并留书面记录**，无 P0/P1 遗留；外部隐私评审签字记入待办，不阻塞 | 自查周 |

**pilot 完成标准（原「上线门」，对齐 PRD 12.9；pilot 阶段不设门禁，未达标不阻塞推进）**：W4-1/2/3 全绿 + D2/D4 团队内部完成（D2 团队自撰并在页面标注，D4 团队内部自查留书面记录）+ 错题原文 100% 不出域断言绿（**CI 断言自动化记入待办**，不阻塞）。
...
| D2 | 监护人授权文案 v1.5（板块二专款）——**〔pilot 口径〕团队自撰**，并在页面显著位置标注「学生团队开发，未经专业法律审核」；正式法律意见记入待办 | 产品（团队自撰） | M3 完成标准前（未达标不阻塞） |
| D3 | 复盘生成内容每周人工抽检流程建立 | 运营 | M3 起持续 |
| D4 | 出域边界 12.6 强化版——**〔pilot 口径〕团队内部自查并留书面记录**；外部隐私评审签字记入待办 | 团队（内部自查） | M3 完成标准前（未达标不阻塞） |
...
| 正式内容清单延期（D1） | 中 | 示例种子可撑演示；M3 完成标准不强制正式内容，但对外演示话术需注明 |
| 团队内部自查延期（D2/D4） | 中 | **〔pilot 口径〕**D2/D4 已由外部法务评审降级为团队内部完成（团队自撰/团队自查并留书面记录），**延期不阻塞进度**；功能可开关关闭（`knowledge_ai_egress_enabled` 默认关），pilot 上线与其余项推进不受影响。<br>**等级保留「中」而非降为「低」**：这里标注的是风险本身，不是阻塞程度。延期意味着产品带着未获专业审核的文案与出域边界运行，这是被主动接受的已知风险，量级仍是中；「不阻塞进度」由本列后半句表达，不靠压低等级来暗示 |
