...
4. **保留监护人授权接口**（取 v1）：PRD 8.1/第 9 节将其列为可上线的最低要求，不能只在 403 层体现；接口收敛为提交/确认/撤销 3 个，不再展开。**〔pilot 口径〕**此处「可上线」指 **pilot 上线**：监护人授权属**设计底线**，接口设计本身（提交/确认/撤销 3 个接口）不变；配套的授权文案按 pilot 第二档执行——由团队自撰，并在页面显著位置标注「学生团队开发，未经专业法律审核」，正式法律意见记入待办、不阻塞。
