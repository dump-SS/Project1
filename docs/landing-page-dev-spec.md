...
| 滚动行为 | 下滑收起、上滑弹出（上滑阈值 3px）。**⚠️ 锁定区间内禁用该行为**（滚轮被吃掉会导致抖动），区间内顶栏常显：① 信任屏横向劫持区间；② 第二屏跑道起点 → 梯形卡片收起点——**顶栏锁定延后到与第二屏梯形卡片同步收起**（Skyer 2026-09-25 口头指定），卡片开始左滑出屏幕的同一刻解除锁定；③ **触底自动弹出**（Skyer 2026-09-25）：`scrollY + innerHeight ≥ scrollHeight − 4` 时强制常显，免得在页尾没有导航可用（离开底部后行为照旧）。多数区间按 owner 注册（`lib/navScrollGuard.ts`） |
| 页尾对比度 | 滚到页尾（亮色流体渐变）时**加深遮罩**，不切文字颜色 |
| 可达性 | hover 之外支持 **click + focus 同效**、`Esc` 关闭、焦点管理；移动端点击展开 |

验收：⌨️ 键盘可走完三个菜单；滚动到信任屏期间顶栏不抖；页尾区文字对比度 ≥ 4.5:1；虚化降级生效。
...
验收：键盘可展开每张卡；移动端不折叠；横滚能进能出；劫持区间顶栏不抖。
...
- **紧凑上移（后续调整）**：拉到底时 **① 末句落在视口顶下方约 79px**（顶栏 64px「多一点点的距离」）、**② 与上一屏的分割线必须已滚出视口**。算式：末句在底部的视口位置 = 视口高 − 172.4（末句→CTA 底：句盒 62.4 + 间距 24 + 胶囊 56 + 下留白 30）− 页尾高；故 `.footer { min-height: max(360px, calc(100dvh − 252px)) }`、`.cta { padding-block: 120px 30px }`（上留白 120 > 目标间距 79，分割线因此落在视口上沿之外；实测末句 top = 79.4、输入框 165.8、卡片上边沿 251.8、分割线 −29.6 不可见）。
- **末句↔其下整簇的两次微调**（Skyer 2026-09-25）：先收「末句↔输入胶囊」间距 34 → 24px 并把页尾 min-height 同步 −10px，使输入框/卡片上边沿/页尾文字整体上提 10px 而末句不动；再按「末句稍往上挪，视觉不协调」把 min-height 算式 264 → 252，末句连同其下整簇再上提 12px。**关键**：末句位置只由页尾 min-height 决定，间距只决定它下面的元素；两者必须成对改，否则末句会跟着漂。
