...
| 滚动行为 | 下滑收起、上滑弹出（上滑阈值 3px）。**⚠️ 锁定区间内禁用该行为**（滚轮被吃掉会导致抖动），区间内顶栏常显：① 信任屏横向劫持区间；② 第二屏跑道起点 → 梯形卡片收起点——**顶栏锁定延后到与第二屏梯形卡片同步收起**（Skyer 2026-09-25 口头指定），卡片开始左滑出屏幕的同一刻解除锁定。多区间按 owner 注册（`lib/navScrollGuard.ts`） |
...
- **紧凑上移（第二次调整）**：拉到底时 **① 末句落在视口顶下方约 92px**（顶栏 64px「多一点点的距离」）、**② 与上一屏的分割线必须已滚出视口**。算式：末句在底部的视口位置 = 视口高 − 172.4（末句→CTA 底：句盒 62.4 + 间距 24 + 胶囊 56 + 下留白 30）− 页尾高；故 `.footer { min-height: max(360px, calc(100dvh − 264px)) }`、`.cta { padding-block: 120px 30px }`（上留白 120 > 目标间距 92，分割线因此落在视口上沿之外）。
- **末句不动、其下整体上提 10px**（Skyer 2026-09-25「页尾文字往上挪一点点，输入框和页底上边沿同步调整」）：靠收「末句↔输入胶囊」间距实现——34 → 24px（−10），页尾 min-height 同步 −10px（−274 → −264）以保持末句仍在 92px；于是输入框（188→177.8）、卡片上边沿（274→263.8）、页尾 logo/版权/合规声明（396→385.8）**同步上提 10px**，而两处间距（句↔胶囊 24、胶囊↔卡片 30）保持不变，末句与分割线位置不受影响（分割线仍 −29.6 在视口外）。
