...
- 链接组 = **四个「品类母项」单行并置**（Skyer 2026-09-25：原 2×2 网格改成横行并置；母项**不可点击**，具体子项未定）：渲染成 `<span>` 而非 `<a>`，保留虚线占位样式以示「结构留位、内容留空」；`.links { display: flex; flex-wrap: wrap; gap: 10px 28px }`（窄屏自动折行）。**下划线与间距**（Skyer 2026-09-25 二次调整：「拉开间距、下划线长度统一、左端与文字对齐、右端长于文字」）：不用 `border-bottom`（长度随文字长短变化），改固定等宽盒 `width: 96px` + 绝对定位 `::after` 画满整盒——文字左对齐，下划线左端自然贴文字、右侧余量就是「长于文字」；`.links` 的 `gap` 由 `10px 28px` 放大到 `12px 42px`。实测四项均 96px 宽、下划线 `width: 96px / left: 0` 全一致，文字宽 57/57/72/57px（即右端分别出格 39/39/24/39px），整组仍单行（组宽 327 → 510px），`clickable: false`。调下划线长度只需改那个 96px。**母项 + 子项结构**（Skyer 2026-09-27）：「隐私协议」不再是独立母项，改挂到「服务条款」下（子项：隐私协议 / 用户协议）；中文三项 = 服务条款（含两子项）/ 社区与文档 / 联系我们，英文同构 = Terms of Service（Privacy Policy / User Agreement）/ Community & Docs / Contact us。`.linkGroup` 纵向排列母项与 `.linkSubList`，`.links` 改 `align-items: flex-start`（有子项的母项更高，顶部对齐才不错位）。**hover = 下划线从左生长 + 文字转深色高亮**：静息那条虚线是「品类占位」标记（母项统一 96px、子项统一 84px + 缩进 12px + 字号 13px），生长条用 `::before { transform: scaleX(0); transform-origin: left }` → hover `scaleX(1)`（0.34s，与顶栏 mega 面板的面板项同一手法），压在虚线上、不替换它。实测真实指针：hover 母项 → 自身 color 转 `rgb(15,21,32)` 且 `::before` 为 `matrix(1,0,0,1,0,0)`、其它项保持静息；hover 子项同理且母项回落。**母项/子项分级**（Skyer 2026-09-27 二次指定）：**母项**灰色 `rgba(15,21,32,0.55)`、**完全无 hover 效果**（不生长、不变色）、保留静息虚线作品类占位标记；**子项**纯黑 `#0f1520`、**静息无下划线**、hover 时 `::before` 下划线从左生长（0.34s）、**与母项文字左对齐**（去掉缩进）；母项 → 首个子项间距由 6px 拉到 16px（`.linkGroup` 的 gap），子项之间 6px。实测：母项 hover 前后状态逐项相同（`grow: none`、色值不变）；子项 hover 后 `::before` 由 `matrix(0,…)` 开始生长；五项文字 `left` 全为 718（严格左对齐）；母项到首个子项 top 差 48px ≈ 盒高 + 16px。子项定下来后挂下一级入口，母项与子项本身仍不做可点入口。
