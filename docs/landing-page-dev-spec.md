...
- 链接组 = **四个「品类母项」单行并置**（Skyer 2026-09-25：原 2×2 网格改成横行并置；母项**不可点击**，具体子项未定）：渲染成 `<span>` 而非 `<a>`，保留虚线占位样式以示「结构留位、内容留空」；`.links { display: flex; flex-wrap: wrap; gap: 10px 28px }`（窄屏自动折行）。实测四项同一行（y 均 324）、组宽 327px、`clickable: false`。子项定下来后挂下一级入口，母项本身仍不做可点入口。
