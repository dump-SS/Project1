    ...
    build: {
      rollupOptions: {
        output: {
          ...
          // three，来自 ColorBends）单独成 chunk，只被落地页动态 import 引用，
          // 保证不进主包（dev-spec §2.3）。TextType 已适配为 CSS 光标，gsap 未引入。
          manualChunks(id: string) {
            if (/[\\/]node_modules[\\/](gsap|ogl|motion|three)[\\/]/.test(id)) return 'landing-gfx';
            return undefined;
          },
        },
      },
