/**
 * Grainient —— 源自 React Bits（https://reactbits.dev，MIT + Commons Clause：
 * 产品内可用，禁止打包成组件库转售），-TS-TW 变体源码拷贝进仓库。
 * 用于落地页页尾流体渐变（visual-language §7.8：全站唯一"大面积 + 鲜明"特例区）。
 *
 * 本仓库改动（2026-09-25）：
 * 1. React 18 兼容：无 React 19 API（纯 hooks），✓；
 * 2. 新增指针交互（原版没有）：pointermove → ref 缓存 → 渲染循环里直接写
 *    uCenterOffset/uZoom uniform（不触发 React 重渲染）；触屏无此交互；
 * 3. 新增 prefers-reduced-motion：静态单帧渲染（不启动 rAF 循环）；
 * 4. 保留原版工程化：IntersectionObserver 离屏暂停 + visibilitychange 暂停 +
 *    ResizeObserver 尺寸同步 + WebGL context 释放。
 */
...
/** 指针视差：[-1,1] 归一化位置，缓动后写 uniform */
type PointerState = { tx: number; ty: number; x: number; y: number }
const pointerMap = new WeakMap<HTMLDivElement, PointerState>()
...
const Grainient: React.FC<GrainientProps> = ({
  ...
  // 最新 props 由渲染循环直接读取（指针交互不触发重渲染）
  const propsRef = useRef({
    timeSpeed, centerX, centerY, zoom,
  })
  propsRef.current = { timeSpeed, centerX, centerY, zoom }
  ...
  useEffect(() => {
    ...
    pointerMap.set(container, { tx: 0, ty: 0, x: 0, y: 0 })
    ...
    // 指针交互：ref 缓存目标位置（触屏无 hover，不注册）
    const onPointerMove = (e: PointerEvent) => {
      const p = pointerMap.get(container)
      if (!p) return
      const rect = container.getBoundingClientRect()
      p.tx = ((e.clientX - rect.left) / rect.width) * 2 - 1
      p.ty = ((e.clientY - rect.top) / rect.height) * 2 - 1
    }
    ...
    const loop = (t: number) => {
      const p = pointerMap.get(container)
      if (p) {
        // 缓动追踪指针（0.04/frame ≈ 轻微拖曳感）
        p.x += (p.tx - p.x) * 0.04
        p.y += (p.ty - p.y) * 0.04
        const off = u.uCenterOffset.value as Float32Array
        off[0] = propsRef.current.centerX + p.x * 0.12
        off[1] = propsRef.current.centerY + p.y * 0.12
        u.uZoom.value = propsRef.current.zoom + p.x * 0.02
      }
      u.iTime.value = (t - t0) * 0.001
      u.uTimeSpeed.value = propsRef.current.timeSpeed
      renderer.render({ scene: mesh })
    ...
    const tryStart = () => {
      if (staticFrame) {
        renderer.render({ scene: mesh }) // reduced-motion：只渲染一帧
        return
      }
