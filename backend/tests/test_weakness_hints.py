...
def _seed():
    ...
    try:
        ...
        db.add(KnowledgePointORM(
            id="kp_2", subject_code="SX", code="math.func.mono", name="函数单调性",
            definition="随自变量增减的性质", parent_id="kp_1", difficulty=3, exam_weight=0.05,
        ))
        db.add(GoalORM(
            id="g_weak", user_id="u_test_weak", type="short_term", subject="SX",
...
def test_graph_weak_point_ids():
    r = client.get("/api/v1/knowledge/subjects/SX/graph", headers=HDR)
...
def test_graph_weak_point_ids_empty_when_no_mastery():
    # 换一个无错题记录的用户，应返回空数组
    r = client.get("/api/v1/knowledge/subjects/SX/graph", headers={"X-User-ID": "u_other"})
    assert r.status_code == 200
