...
def test_recommendation_feedback_persists():
    ...
    put = client.put(f"/recommendations/{rec_id}/feedback", json={"rating": "useful", "reason": "很具体"})
    assert put.status_code == 200
    assert put.json()["feedback"]["rating"] == "useful"

    got = client.get(f"/recommendations/{rec_id}")
...
def test_summary_insufficient_data_is_not_template():
    ...
    r = client.post("/summaries", json={"periodStart": "2030-01-01", "periodEnd": "2030-01-07"})
    assert r.status_code == 202
    sum_id = r.json()["summaryId"]

    got = client.get(f"/summaries/{sum_id}")
...
def test_summary_mock_provider_fails_not_template():
    ...
    r = client.post("/summaries", json={"periodStart": "2026-08-12", "periodEnd": "2026-08-16"})
    assert r.status_code == 202
    got = client.get(f"/summaries/{r.json()['summaryId']}")
    body = got.json()
